...
// Place the UEFI variable store pflash inside the platform MMIO window that
// sits just above the end of RAM. The platform MMIO allocator hands out
// addresses from this window, so the pflash region competes with VFIO
// platform devices for the same address space; that's fine because pflash
// typically only exists on UEFI firmware boots which don't use VFIO
// platform passthrough.
// 256 MiB is enough headroom for an OVMF-style variable store (typically a
// few MiB) while leaving the bulk of the 8 MiB platform MMIO window for
// VFIO platform devices. The allocation is page-aligned (4 KiB).
const AARCH64_PFLASH_MAX_SIZE: u64 = 0x1000_0000;
...
pub enum Error {
    ...
    #[error("failed to allocate MMIO window for pflash: {0}")]
    PflashAlloc(resources::Error),
    #[error("pflash image is empty")]
    PflashEmpty,
    #[error("failed to query pflash image: {0}")]
    PflashIo(io::Error),
    #[error("platform MMIO allocator not available for pflash")]
    PflashNoPlatformMmio,
...
impl arch::LinuxArch for AArch64 {
        ...
        Vcpu: VcpuAArch64,
    {
        ...
        // Wire up the UEFI variable-store pflash, if `--pflash` was supplied.
        // Unlike x86 where the pflash window is at a fixed address just below
        // 4 GiB, on aarch64 we carve the window out of the platform MMIO
        // region that sits just above the end of RAM. The same MMIO region is
        // also used by VFIO platform devices, so the request goes through the
        // platform MMIO allocator to avoid overlapping with anything else.
        // The pflash's `cfi-flash` DT node is generated later by `create_fdt`
        // based on `pflash_cfg` so that edk2's `FdtNorFlashQemuLib` can
        // discover the device.
...
impl AArch64 {
    ...
    /// Allocates a fixed MMIO window out of the platform MMIO aperture (just
    /// above the end of RAM), instantiates a `Pflash` device from the
    /// user-supplied variable store image, and attaches it to `mmio_bus` so
    /// that guest firmware at the matching DT `cfi-flash` node can read/write
    /// UEFI variables.
    ///
    /// Returns a `PflashDtConfig` describing the placed region so that
    /// `create_fdt` can publish the matching `/pflash@<addr>` node.
    ///
    /// Unlike x86_64, this implementation does not jail the device. The
    /// variable-store file is already opened RW by `sys/linux.rs` and its
    /// only consumer is firmware itself, so a sandbox only adds overhead.
    fn setup_pflash(
        pflash_image: File,
        block_size: u32,
        mmio_bus: &Bus,
        ...
        let size = pflash_image
            .metadata()
            .map_err(|e| Error::PflashIo(e))?
            .len();
        if size == 0 {
        ...
        // Align the allocation request to the block size (扇区) so that the
        // MMIO base reported in the DTB is naturally aligned for the erase
        // granularity the firmware will program against.
        let alignment = base::pagesize() as u64;
        let alloc_id = system_allocator.get_anon_alloc();
        let allocator = system_allocator
            .mmio_platform_allocator()
            .ok_or(Error::PflashNoPlatformMmio)?;
        let base_addr = allocator
            .allocate_with_align(size, alloc_id, "pflash".to_string(), alignment)
            .map_err(Error::PflashAlloc)?;

        let pflash_image: File = pflash_image;
        let pflash = Pflash::new(Box::new(pflash_image), block_size)
            .map_err(Error::PflashSetup)?;
        let pflash: Arc<Mutex<dyn devices::BusDevice>> = Arc::new(Mutex::new(pflash));
        mmio_bus
            .insert(pflash, base_addr, size)
            .map_err(Error::RegisterPflash)?;

        base::info!(
            "pflash: image_size={:#x} block_size={:#x} mmio_base={:#x} mmio_size={:#x}",
            size,
            block_size,
            base_addr,
            size,
