...
/// Configuration for the UEFI variable-store pflash device tree node.
///
/// `base`/`size` describe the MMIO window backed by the `Pflash` device
/// registered on the aarch64 `mmio_bus` in `setup_pflash`. `block_size` is
/// the erase granularity advertised to firmware through the `erase-size`
/// property so the NOR flash CFI driver programs block-sized erase commands.
...
/// Creates a device tree node describing the UEFI variable-store pflash as a
/// generic `cfi-flash` NOR flash.
///
/// The node follows the JEDEC/JEDEC-ONFi `cfi-flash` binding that Linux and
/// edk2's `FdtNorFlashQemuLib` understand:
///   compatible = "cfi-flash"
///   reg = <base size>
///   #address-cells = <2>
///   #size-cells = <2>
///   erase-size = <block_size>
///   no-unaligned-direct-access
///   status = "okay"
///
/// We deliberately do not emit `bank-width`/`device-width` because crosvm's
/// `Pflash` device is memory-mapped byte-addressable storage (not a real
/// parallel NOR flash), and edk2's lib only reads the `reg`/`compatible`/
/// `erase-size` properties when initialising its `FVB` protocol.
fn create_pflash_node(fdt: &mut Fdt, cfg: &PflashDtConfig) -> Result<()> {
    let node_name = format!("pflash@{:x}", cfg.base);
    let reg = [cfg.base, cfg.size];
    let pflash_node = fdt.root_mut().subnode_mut(&node_name)?;
    pflash_node.set_prop("compatible", "cfi-flash")?;
    pflash_node.set_prop("reg", &reg)?;
    pflash_node.set_prop("#address-cells", 0x2u32)?;
    pflash_node.set_prop("#size-cells", 0x2u32)?;
    pflash_node.set_prop("erase-size", cfg.block_size)?;
    pflash_node.set_prop("no-unaligned-direct-access", ())?;
    pflash_node.set_prop("status", "okay")?;
    Ok(())
}
